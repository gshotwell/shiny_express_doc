...
Traditional Shiny apps are _bifurcated_ in that they require you explicitly define a UI object and a server funciton. 
By contrast, Shiny Express allows you to intermingle ui and server code.
...
Contrast the Express code above with the full featured Shiny equivalent below:
...
- In a bifurcated app, we first used the page-level component `ui.page_sidebar()` with a `ui.sidebar()` nested inside.
...
While we believe that Shiny Express will turn out to be an effective tool for those new to Shiny, it is not appropriate for all use cases. A recurring theme you will find in the finer points below is that Shiny Express is easier to write but harder to manipulate and reason about, while traditional Shiny apps demand more up-front learning and some small inconveniences in return for being easier to read and reason about as your apps get larger.
...
The main differences between Express apps and traditional Shiny apps are the following:
...
Traditional Shiny apps require that  UI and server logic be declared separately, with the UI containing `ui.output_xxx` objects to indicate _where_ each output goes and the server containing the logic indicating _what_ each output is.
...
In Shiny, all layout components are available from the `ui` submodule, for example, `ui.sidebar()`, and `ui.input_text()`. You might create a simple app UI like this:
...
In traditional Shiny apps, the UI always has a top-level `page_` container, like `page_fluid()`, `page_fillable()`, or `page_sidebar()`.
